...
mod tests {
    ...
    fn test_random_bytes() {
        for _ in 0..1000 {
            let mut buf = BytesMut::with_capacity(1024);
            let mut size = (rand::random::<char>() as usize) % 1024;

            for i in 0..size {
                buf.put_u8(rand::random::<char>() as u8);
            }
            println!("Testing random bytes with len {}...", size);

            //assert_eq!(packet.data, buffer);

            let hellopkg = MinecraftHelloPacket::new(&mut buf);
            match hellopkg {
                Ok(hello) => {
